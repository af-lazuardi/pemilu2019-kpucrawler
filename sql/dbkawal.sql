...
--
-- Database: `kjamid_dbkawal`
--
CREATE DATABASE IF NOT EXISTS `kjamid_dbkawal` DEFAULT CHARACTER SET latin1 COLLATE latin1_swedish_ci;
USE `kjamid_dbkawal`;
...
CREATE ALGORITHM=UNDEFINED  SQL SECURITY DEFINER VIEW `v01_kota`  AS  select `t_kota`.`kdkota` AS `kdkota`,`t_kota`.`kota` AS `kota`,`t_kota`.`kdprov` AS `kdprov`,`t_prov`.`prov` AS `prov` from (`t_kota` join `t_prov` on((`t_kota`.`kdprov` = `t_prov`.`kdprov`))) ;
...
CREATE ALGORITHM=UNDEFINED  SQL SECURITY DEFINER VIEW `v02_kec`  AS  select `t_kec`.`kdkec` AS `kdkec`,`t_kec`.`kec` AS `kec`,`t_kec`.`kdkota` AS `kdkota`,`v01_kota`.`kota` AS `kota`,`v01_kota`.`kdprov` AS `kdprov`,`v01_kota`.`prov` AS `prov` from (`t_kec` join `v01_kota` on((`t_kec`.`kdkota` = `v01_kota`.`kdkota`))) ;
...
CREATE ALGORITHM=UNDEFINED  SQL SECURITY DEFINER VIEW `v13_tps_sumcmp`  AS  select sum(`v11_tps_cmponly`.`cmp1`) AS `cmp1`,sum(`v11_tps_cmponly`.`cmp2`) AS `cmp2`,count(0) AS `jumlah` from `v11_tps_cmponly` ;
...
CREATE ALGORITHM=UNDEFINED  SQL SECURITY DEFINER VIEW `v31_kpupantau_cmponly`  AS  select `A`.`kdtps` AS `kdtps`,`A`.`kdkel` AS `kdkel`,`A`.`nama` AS `nama`,`A`.`no` AS `no`,`A`.`kpu1` AS `kpu1`,`A`.`kpu2` AS `kpu2`,`A`.`pnt1` AS `pnt1`,`A`.`pnt2` AS `pnt2`,`A`.`cmp1` AS `cmp1`,`A`.`cmp2` AS `cmp2` from `v30_kpupantau_cmp` `A` where ((`A`.`cmp1` <> 0) or (`A`.`cmp2` <> 0)) ;
...
CREATE ALGORITHM=UNDEFINED  SQL SECURITY DEFINER VIEW `v50_sum_data`  AS  select (select count(0) from `t_tps`) AS `tps_total`,(select count(0) from `t_tps` where ((`t_tps`.`kpu1` is not null) or (`t_tps`.`kpu2` is not null))) AS `tps_masuk`,(select count(0) from `v25_invalid_kpu_data_formatted`) AS `tps_invalid` ;
...
CREATE ALGORITHM=UNDEFINED  SQL SECURITY DEFINER VIEW `v50_sum_data_persen`  AS  select `A`.`tps_total` AS `tps_total`,`A`.`tps_masuk` AS `tps_masuk`,`A`.`tps_invalid` AS `tps_invalid`,((`A`.`tps_masuk` / `A`.`tps_total`) * 100) AS `persen_masuk`,((`A`.`tps_invalid` / `A`.`tps_masuk`) * 100) AS `persen_invalid` from `v50_sum_data` `A` ;
